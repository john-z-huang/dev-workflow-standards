...
- **Issue 与 PR 流程** — 功能/修复须创建 Issue；创建时优先读取仓库已有 labels、附加至少一个工作类型分类标签并默认指派给当前已认证的本人账号；若已授权集成层无法枚举 labels，但支持创建时写入与创建后回读，则可使用受限的默认类别候选并以实际写入结果验证，不因缺少枚举接口本身阻塞；PR 创建时同样默认自指派并关联 Issue
