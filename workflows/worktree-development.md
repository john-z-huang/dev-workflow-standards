...
操作细节：[`worktree.md`](../references/worktree.md)；Fork 拓扑：[`fork-maintenance.md`](../references/fork-maintenance.md)；依赖拓扑：[`stacked-pr.md`](../references/stacked-pr.md)；分支命名：[`check-branch-name.md`](../references/check-branch-name.md)；提交信息：[`check-commit-message.md`](../references/check-commit-message.md)。原始工作区只同步主分支和管理 worktree；需求实现、验证、暂存、提交在需求 worktree 中完成。
...
1. 确认 remote、实际默认主分支和当前分支；个人 Fork 先读取 [`fork-maintenance.md`](../references/fork-maintenance.md)。
2. 确认没有妨碍切换/拉取的已跟踪未提交改动。
3. 安全切换到主分支；普通仓库执行一次 `git pull --ff-only origin <main>`，个人 Fork 执行 `git fetch upstream <main>` 后以 `git merge --ff-only upstream/<main>` 同步，并按 Fork 规则更新 `origin/<main>`。
4. 同步失败、非 fast-forward、认证/网络失败、未跟踪冲突或权限不足：停止，不创建 worktree。
...
- 从刚同步的本地主分支创建：普通仓库使用 `main`；个人 Fork 的需求 worktree 使用刚同步的个人维护分支作为起点。
...
多个 worktree 的交付顺序：一个 worktree 验证、提交、推送、建 PR并等待合并 → 原始工作区同步实际 base → 下一个 worktree 在自身目录 rebase、重新验证、再交付。个人 Fork 的需求 base 是个人维护分支，不是 `main`。已推送分支 rebase 仅在获授权时用 `--force-with-lease`，不用裸 `--force`。

## 8. 合并后状态

确认合并后只在原始工作区同步实际 base 分支；worktree 工作流默认不 checkout/pull/rebase/delete 已合并 worktree/分支，也不恢复或删除远端 head。需清理时必须用户明确要求。未跟踪文件先同步对应基线，再按清单和指纹恢复，worktree 仍保留。
