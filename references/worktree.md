...
启用本文后，确认仓库实际使用的 remote 和默认主分支；示例使用 `origin` 和 `main`，项目使用其他名称时以实际配置为准。若没有 Git 仓库或没有配置 remote，仍不能凭本文强制创建 worktree；按用户明确要求和项目已有流程处理，并在缺少同步前提时停止或报告。

在已选定 worktree 模式后，每个完整需求在一个独立 worktree 中实现、验证和提交，并使用该需求自己的工作分支。只有需求确实要并行拆分成多个可独立审查的工作项时，才为各工作项分别创建 worktree 和分支。无论并行 worktree 来自同一需求的拆分，还是来自多个独立需求，只要它们最终都要集成到同一主分支，就必须按本文队列逐个推送、合并、同步主分支、rebase 下一个工作项并重新验证。
