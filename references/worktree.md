...
2. 集成确认后，只回到原始工作区，将主分支快进同步到 remote 的最新状态。仅在原始工作区状态允许安全切换时执行，例如：
...
本流程不自动删除已合并需求的 worktree 或本地分支。若该需求 worktree 携带了“不涉及本次需求”的未跟踪文件，必须先在原始工作区将 `main` 从远端同步成功，再按“原始工作区存在未跟踪文件时”一节的清单和冲突校验从 worktree 恢复这些文件；恢复完成后仍保留 worktree，除非用户另行要求清理。只有用户另行明确要求清理时，才按其指定范围处理，并遵守现有 Git 授权。
